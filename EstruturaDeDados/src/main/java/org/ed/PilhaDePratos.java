...
public class PilhaDePratos implements IEstruturaDeDados {
    ...
    public void adicionar(int valor) { // push()
        if (topo == elementos.length - 1) {
            throw new RuntimeException("A pilha de prato está cheia!");
        }
    ...
    public char remover() { // pop()
        if (estaVazia()) {
            throw new RuntimeException("A pilha de prato está vazia!");
        }
